...
/*************************************
 * note by Yicheng: I/O rewritten 20230922
 *************************************/
/*************************************
 * \author Mike Lisa
 * \date 23 June 2018
...
class StEpdEpFinder{
 public:
  ...
  StEpdEpFinder(TString dir="", TString name="", int nEventTypeBins=10);
  ~StEpdEpFinder(){/* no-op */};

  void SetName(TString Name);
  void SetDir(TString Dir);
  void SetRefName(TString RefName);
  void SetRefDir(TString RefDir);

  TString GetName() const;
  TString GetDir() const;
  TString GetRefName() const;
  TString GetRefDir() const;

  void LoadCorrectionFile(TString filepath);
  TString mCorrectionFilePath;
  TString GetCorrectionFilePath() const;
  ...
  void SetRingWeights(int order, std::vector<double> RingWeights);
  ...
  StEpdEpInfo Results(TClonesArray* EpdHits, TVector3 primVertex, int EventTypeID, double EventWeight, double RefPsi1);
  ...
  int GetWeightingScheme() const;
  TH2D GetEtaWeights(int order) const;
  std::vector<double> GetRingWeights(int order) const;
  bool GetIsEtaWeighted(int order) const;
  bool GetIsRingWeighted(int order) const;
 ...
 private:
  ...
  bool OrderOutsideRange(int order) const;         // just makes sure order is between 1 and _EpOrderMax
  ...
  bool mIsEtaWeighted[_EpOrderMax];
  bool mIsRingWeighted[_EpOrderMax];

  TProfile* mAveCosDeltaPsi[_EpOrderMax];        // average of cos(Psi_{East,n}-Psi_{West,n}) using phi-weighted and shifted EPs
  TH1D* mRawEpdPsiDist[3][_EpOrderMax];
  TH1D* mCorEpdPsiDist[3][_EpOrderMax];
  TH1D* mShfEpdPsiDist[3][_EpOrderMax];
  TH2D *mRawEpdSubsPsiDist[_EpOrderMax];
  TH2D *mCorEpdSubsPsiDist[_EpOrderMax];
  TH2D *mShfEpdSubsPsiDist[_EpOrderMax];

  TProfile2D *mEtaRawV1;
  TProfile2D *mRingRawV1;
  TProfile2D *mRingEta;
...
inline double StEpdEpFinder::GetnMipThreshold() const {return mThresh;}
inline double StEpdEpFinder::GetMaxTileWeight() const {return mMax;}
inline int StEpdEpFinder::GetEpdHitFormat() const {return mFormatUsed;}

inline void StEpdEpFinder::SetName(TString Name) { mName = Name; }
inline void StEpdEpFinder::SetDir(TString Dir) { mDir = Dir; }
inline void StEpdEpFinder::SetRefName(TString RefName) { mRefName = RefName; }
inline void StEpdEpFinder::SetRefDir(TString RefDir) { mRefDir = RefDir; }

inline TString StEpdEpFinder::GetName() const { return mName; }
inline TString StEpdEpFinder::GetDir() const { return mDir; }
inline TString StEpdEpFinder::GetRefName() const { return mRefName; }
inline TString StEpdEpFinder::GetRefDir() const { return mRefDir; }

inline TString StEpdEpFinder::GetCorrectionFilePath() const { return mCorrectionFilePath; }

inline int StEpdEpFinder::GetWeightingScheme() const { return mWeightingScheme; }
