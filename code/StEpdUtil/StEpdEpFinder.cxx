...
//==================================================================================================================
StEpdEpFinder::StEpdEpFinder(TString dir, TString name, int nEventTypeBins) : mFormatUsed(2), mThresh(0.3), mMax(2.0), mWeightingScheme(0)
{
...
//       << "*  To include higher orders, edit StEpdUtil/StEpdEpInfo.h\n**********\n";
  ...
  mDir = dir;
  mName = name;
  std::cout << "StEpdEpFinder: " << mName << std::endl;
  std::cout << "StEpdEpFinder: nEventTypeBins = " << nEventTypeBins << ", EpdOrderMax = " << _EpOrderMax << std::endl;
  ...
  for (int iOrder=0; iOrder<_EpOrderMax; iOrder++){
    ...
    mIsEtaWeighted[iOrder] = false;
    mIsRingWeighted[iOrder] = false;
  }
  ...
  for (int ew=0; ew<2; ew++){
    for (int order=1; order<_EpOrderMax+1; order++){
      mEpdShiftOutput_sin[ew][order-1] = new TProfile2D(mName+Form("EpdShiftEW%dPsi%d_sin",ew,order), Form("EpdShiftEW%dPsi%d_sin",ew,order),
							_EpTermsMax,0.5,1.0*_EpTermsMax+.5, nEventTypeBins,-0.5,(double)nEventTypeBins-0.5, -1.0,1.0);
      mEpdShiftOutput_cos[ew][order-1] = new TProfile2D(mName+Form("EpdShiftEW%dPsi%d_cos",ew,order), Form("EpdShiftEW%dPsi%d_cos",ew,order),
							_EpTermsMax,0.5,1.0*_EpTermsMax+.5, nEventTypeBins,-0.5,(double)nEventTypeBins-0.5, -1.0,1.0);
    }
    mPhiWeightOutput[ew]   = new TH3D(mName+Form("PhiWeightEW%d",ew),Form("Tile Weight divided by Averaged EW=%d",ew),12,0.5,12.5,31,0.5,31.5,nEventTypeBins,-0.5,(double)nEventTypeBins-0.5); // bins are PP,TT,EventType
    mPhiAveraged[ew]       = new TH3D(mName+Form("PhiAveraged%d",ew),Form("Average for this phi EW=%d",ew),12,0.5,12.5,31,0.5,31.5,nEventTypeBins,-0.5,(double)nEventTypeBins-0.5); // just for normalization. discard after use
  }
  for (int order=1; order<_EpOrderMax+1; order++){
    mEpdShiftOutput_sin[2][order-1] = new TProfile2D(mName+Form("EpdShiftFullEventPsi%d_sin",order),
						     Form("EpdShiftFullEventPsi%d_sin",order),
						     _EpTermsMax,0.5,1.0*_EpTermsMax+.5,nEventTypeBins,-0.5,(double)nEventTypeBins-0.5,-1.0,1.0);
    mEpdShiftOutput_cos[2][order-1] = new TProfile2D(mName+Form("EpdShiftFullEventPsi%d_cos",order),
  ...
  for (int order=1; order<=_EpOrderMax; order++){
    mAveCosDeltaPsi[order-1] = new TProfile(mName+Form("AveCosDeltaPsi%d",order),
    ...
    for (int ewFull=0; ewFull<3; ewFull++) {
      mRawEpdPsiDist[ewFull][order-1] = new TH1D(mName+Form("Ewf%dRawEpdPsi%dDist",ewFull,order), mName+Form("Ewf%dRawEpdPsi%dDist",ewFull,order), 200,0,2*TMath::Pi()/order);
      mCorEpdPsiDist[ewFull][order-1] = new TH1D(mName+Form("Ewf%dCorEpdPsi%dDist",ewFull,order), mName+Form("Ewf%dCorEpdPsi%dDist",ewFull,order), 200,0,2*TMath::Pi()/order);
      mShfEpdPsiDist[ewFull][order-1] = new TH1D(mName+Form("Ewf%dShfEpdPsi%dDist",ewFull,order), mName+Form("Ewf%dShfEpdPsi%dDist",ewFull,order), 200,0,2*TMath::Pi()/order);
    }
    mRawEpdSubsPsiDist[order-1] = new TH2D(mName+Form("RawEpdSubsPsi%dDist",order), mName+Form("RawEpdSubsPsi%dDist",order), 32,0,2*TMath::Pi()/order, 32,0,2*TMath::Pi()/order);
    mCorEpdSubsPsiDist[order-1] = new TH2D(mName+Form("CorEpdSubsPsi%dDist",order), mName+Form("CorEpdSubsPsi%dDist",order), 32,0,2*TMath::Pi()/order, 32,0,2*TMath::Pi()/order);
    mShfEpdSubsPsiDist[order-1] = new TH2D(mName+Form("ShfEpdSubsPsi%dDist",order), mName+Form("ShfEpdSubsPsi%dDist",order), 32,0,2*TMath::Pi()/order, 32,0,2*TMath::Pi()/order);
  }

  mEtaRawV1 = new TProfile2D(mName+Form("EtaRawV1"), mName+Form("EtaRawV1"), nEventTypeBins,-0.5,(double)nEventTypeBins-0.5, 260,-6.5,6.5);
  mRingRawV1 = new TProfile2D(mName+Form("RingRawV1"), mName+Form("RingRawV1"), nEventTypeBins,-0.5,(double)nEventTypeBins-0.5, 32,-0.5,31.5);
  mRingEta = new TProfile2D(mName+Form("RingEta"), mName+Form("RingEta"), nEventTypeBins,-0.5,(double)nEventTypeBins-0.5, 32,-0.5,31.5);
  mEtaRawV1->Sumw2();
  mRingRawV1->Sumw2();
  mRingEta->Sumw2();
}
...
//==================================================================================================================
void StEpdEpFinder::LoadCorrectionFile(TString CorrectionFile) {

  TString StrDir = "";
  if(mDir!="") StrDir = mDir + "/";
  TString StrDirName = StrDir + mName;
  mCorrectionFilePath = CorrectionFile;
  ...
  if (mCorrectionInputFile->IsZombie()) {
    ...
    std::cout << "Correction files loaded." << std::endl;
    for (int ew=0; ew<2; ew++){
      mPhiWeightInput[ew] = (TH3D*)mCorrectionInputFile->Get(StrDirName+Form("PhiWeightEW%d",ew));
      if(!mPhiWeightInput[ew]) {
        cout << "Invalid PhiWeightEW!!! " << StrDirName << endl;
        if(mRefDir=="") StrDirName = mRefName;
        if(mRefDir!="") StrDirName = mRefDir + "/" + mRefName;
        cout << "Search backup PhiWeightEW: " << StrDirName << endl;
        mPhiWeightInput[ew] = (TH3D*)mCorrectionInputFile->Get(StrDirName+Form("PhiWeightEW%d",ew));
      }
      for (int order=1; order<_EpOrderMax+1; order++){
	mEpdShiftInput_sin[ew][order-1] = (TProfile2D*)mCorrectionInputFile->Get(StrDirName+Form("EpdShiftEW%dPsi%d_sin",ew,order));
	mEpdShiftInput_cos[ew][order-1] = (TProfile2D*)mCorrectionInputFile->Get(StrDirName+Form("EpdShiftEW%dPsi%d_cos",ew,order));
      }
    }
    for (int order=1; order<_EpOrderMax+1; order++){
      mEpdShiftInput_sin[2][order-1] = (TProfile2D*)mCorrectionInputFile->Get(StrDirName+Form("EpdShiftFullEventPsi%d_sin",order));
      mEpdShiftInput_cos[2][order-1] = (TProfile2D*)mCorrectionInputFile->Get(StrDirName+Form("EpdShiftFullEventPsi%d_cos",order));
    }
  }
}
...
void StEpdEpFinder::SetEtaWeights(int order, TH2D EtaWeights){
  ...
  mIsEtaWeighted[order-1] = true;
}

//==================================================================================================================
TH2D StEpdEpFinder::GetEtaWeights(int order) const {
  if (OrderOutsideRange(order)){
    cout << "Order outside range - Ignoring your eta weights\n";
    return TH2D();
  }
  if (mWeightingScheme != 1) {
    cout << "Not weighted by eta!\n";
    return TH2D();
  }
  if (!mIsEtaWeighted[order-1]) {
    cout << "Not weighted by eta!\n";
    return TH2D();
  }
  return *mEtaWeights[order-1];
}

//==================================================================================================================
bool StEpdEpFinder::GetIsEtaWeighted(int order) const {
  if (OrderOutsideRange(order)){
    cout << "Order outside range - Ignoring your eta weights\n";
    return false;
  }
  ...
  return mIsEtaWeighted[order-1];
}

//==================================================================================================================
//void StEpdEpFinder::SetRingWeights(int order, double* RingWeights){
void StEpdEpFinder::SetRingWeights(int order, vector<double> RingWeights){
  if (OrderOutsideRange(order)){
...
//    assert(0);
//  }
  if(RingWeights.size() != 16) {
    cout << "Wrong number of rings!\n";
    return;
  }
  for (int iring=0; iring<16; iring++){
    mRingWeights[iring][order-1] = RingWeights[iring];
  }
  mIsRingWeighted[order-1] = true;
}

//==================================================================================================================
vector<double> StEpdEpFinder::GetRingWeights(int order) const {
  vector<double> tmp;
  if (OrderOutsideRange(order)){
    cout << "Order outside range - Ignoring your eta weights\n";
    return tmp;
  ...
  if (mWeightingScheme != 2) {
    cout << "Not weighted by ring!\n";
    return tmp;
  }
  if (!mIsRingWeighted[order-1]) {
    cout << "Not weighted by ring!\n";
    return tmp;
  }
  for (int iring=0; iring<16; iring++){
    tmp.push_back(mRingWeights[iring][order-1]);
  }
  return tmp;
}

//==================================================================================================================
bool StEpdEpFinder::GetIsRingWeighted(int order) const {
  if (OrderOutsideRange(order)){
    cout << "Order outside range - Ignoring your eta weights\n";
    return false;
  }
  ...
  return mIsRingWeighted[order-1];
}

//==================================================================================================================
void StEpdEpFinder::Finish(TFile *fOut) {
  if(!mCorrectionInputFile->IsZombie()) mCorrectionInputFile->Close();
...
//  mResolutionOutputFile->Close();
  ...
  for (int ew=0; ew<2; ew++){
    for (int order=1; order<_EpOrderMax+1; order++){
      mEpdShiftOutput_sin[ew][order-1]->Write();
      mEpdShiftOutput_cos[ew][order-1]->Write();
    }
    mPhiWeightOutput[ew]->Write();
    //mPhiAveraged[ew]->Write();
  }
  for (int order=1; order<_EpOrderMax+1; order++){
    mEpdShiftOutput_sin[2][order-1]->Write();
    mEpdShiftOutput_cos[2][order-1]->Write();
  }

  for (int order=1; order<=_EpOrderMax; order++){
    mAveCosDeltaPsi[order-1]->Write();
    for (int ewFull=0; ewFull<3; ewFull++) {
      mRawEpdPsiDist[ewFull][order-1]->Write();
      mCorEpdPsiDist[ewFull][order-1]->Write();
      mShfEpdPsiDist[ewFull][order-1]->Write();
    }
    mRawEpdSubsPsiDist[order-1]->Write();
    mCorEpdSubsPsiDist[order-1]->Write();
    mShfEpdSubsPsiDist[order-1]->Write();
  }
...
//==================================================================================================================
StEpdEpInfo StEpdEpFinder::Results(TClonesArray* EpdHits, TVector3 primVertex, int EventTypeId, double EventWeight=1.0, double RefPsi1=-999.0){
  ...
  for (int hit=0; hit<EpdHits->GetEntries(); hit++){
    ...
    TVector3 RandomStraightLine = mEpdGeom->RandomPointOnTile(tileId) - primVertex;
    double random_phi = RandomStraightLine.Phi();
    double random_eta = RandomStraightLine.Eta();

    //---------------------------------
    // fyc: check the status
    // --------------------------------
    IsGood[EW] = true;
    ...
    //---------------------------------
    mPhiWeightOutput[EW]->Fill(PP,TT,EventTypeId,TileWeight*mEventWeight);
    if (TT==1){ for (int pp=1; pp<13; pp++) mPhiAveraged[EW]->Fill(pp,1,EventTypeId,TileWeight/12.0*mEventWeight);}
    else{
      for (int pp=1; pp<13; pp++){
	for (int tt=2*(ring-1); tt<2*ring; tt++) mPhiAveraged[EW]->Fill(pp,tt,EventTypeId,TileWeight/24.0*mEventWeight);
      }
    }
    //--------------------------------
    ...
    //---------------------------------
    // fyc: a raw V1 for EPD Eta weights
    // --------------------------------
    if(RefPsi1>-100.0) mEtaRawV1->Fill(EventTypeId, random_eta, cos(random_phi-RefPsi1), PhiWeightedTileWeight*mEventWeight);
    if(RefPsi1>-100.0) mRingRawV1->Fill(EventTypeId, EW*16+ring-1, cos(random_phi-RefPsi1), PhiWeightedTileWeight*mEventWeight);
    if(RefPsi1>-100.0) mRingEta->Fill(EventTypeId, EW*16+ring-1, random_eta, PhiWeightedTileWeight*mEventWeight);
  }  // loop over hits
  //--------------------------------- end loop over hits ---------------------------------------
  ...
  // fyc: check the status
  // --------------------------------
  result.SetIsGoodEast(IsGood[0]);
  result.SetIsGoodWest(IsGood[1]);
  ...
  for (int order=1; order<=_EpOrderMax; order++){
    mAveCosDeltaPsi[order-1]->Fill((double)EventTypeId,
				   cos(((double)order)*(result.PsiPhiWeightedAndShifted[0][order-1]-result.PsiPhiWeightedAndShifted[1][order-1])), mEventWeight);
    for (int ewFull=0; ewFull<3; ewFull++) {
      mRawEpdPsiDist[ewFull][order-1]->Fill(result.PsiRaw[ewFull][order-1], mEventWeight);
      mCorEpdPsiDist[ewFull][order-1]->Fill(result.PsiPhiWeighted[ewFull][order-1], mEventWeight);
      mShfEpdPsiDist[ewFull][order-1]->Fill(result.PsiPhiWeightedAndShifted[ewFull][order-1], mEventWeight);
    }
    mRawEpdSubsPsiDist[order-1]->Fill(result.PsiRaw[0][order-1], result.PsiRaw[1][order-1], mEventWeight);
    mCorEpdSubsPsiDist[order-1]->Fill(result.PsiPhiWeighted[0][order-1], result.PsiPhiWeighted[1][order-1], mEventWeight);
    mShfEpdSubsPsiDist[order-1]->Fill(result.PsiPhiWeightedAndShifted[0][order-1], result.PsiPhiWeightedAndShifted[1][order-1], mEventWeight);
  }
  ...
  for (int i=1; i<=_EpTermsMax; i++){
    for (int ewFull=0; ewFull<3; ewFull++){
      for (int order=1; order<_EpOrderMax+1; order++){
	double tmp = (double)(order*i);
	mEpdShiftOutput_sin[ewFull][order-1]->Fill(i,EventTypeId,sin(tmp*result.PsiPhiWeighted[ewFull][order-1]), mEventWeight);
	mEpdShiftOutput_cos[ewFull][order-1]->Fill(i,EventTypeId,cos(tmp*result.PsiPhiWeighted[ewFull][order-1]), mEventWeight);
      }
    }
  }
...
//==================================================================================================================
bool StEpdEpFinder::OrderOutsideRange(int order) const {
  if (order < 1) {
